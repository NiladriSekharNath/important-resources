 ...
 *
 * The first line of each test case contains two space-separated integers, ‘N’ and ‘D,’ denoting the number of elements
 * in the array and the desired difference.
 *
 ...
 * 3
 *
 * If we see here we are asked to find the count of subsets with given difference
 *
 * let's we are tasked to find the
 * s1 - s2 = diff
 *
 * now if we can look closely s1 + s2 = totalSum
 *
 * if we add both expressions
 *
 * we get 2s1 = totalSum + diff
 *
 * s1 = (totalSum + diff)/2;
 *
 * Now problem basically reduces to count of subsets equal where sum = s1
 *
 * which we already have done previously
 *
 * now just a point if the (totalSum + diff ) is not divisible by 2 we return 0 as it is impossible to find such a value
 *
 *
 *
 */
public class CountSubsetsPartitionsWithGivenDiff {
  public static int countPartitions(int n, int diff, int[] nums) {
    int totalSum = 0 ;
    for(int num : nums)
      totalSum += num;
    int totalSumDiff = totalSum + diff;
